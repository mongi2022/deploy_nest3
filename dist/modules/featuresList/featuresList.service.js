"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FeaturelistService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const featuresList_entity_1 = require("./entity/featuresList.entity");
let FeaturelistService = class FeaturelistService {
    constructor(featurelistRepository) {
        this.featurelistRepository = featurelistRepository;
    }
    getAllFeaturelist() {
        return this.featurelistRepository.find();
    }
    async getFeaturelistById(id) {
        return await this.featurelistRepository.findOne({ where: { id } });
    }
    async createFeaturelist(data) {
        const featurelist = await this.featurelistRepository.create(data);
        return this.featurelistRepository.save(featurelist);
    }
    async updateFeaturelist(id, data) {
        await this.featurelistRepository.update({ id }, data);
        const newFeaturelist = await this.featurelistRepository.findOne({ where: { id } });
        if (newFeaturelist == null)
            throw new common_1.NotFoundException(`Featurelist n° ${id} n'existe pas`);
        return newFeaturelist;
    }
    async deleteFeaturelist(id) {
        const deleted = await this.featurelistRepository.findOne({ where: { id } });
        if (deleted == null)
            throw new common_1.NotFoundException(`Featurelist n° ${id} n'existe pas`);
        await this.featurelistRepository.delete({ id });
        return deleted;
    }
};
FeaturelistService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(featuresList_entity_1.FeaturelistEntity)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], FeaturelistService);
exports.FeaturelistService = FeaturelistService;
//# sourceMappingURL=featuresList.service.js.map